...
def unit_nodes_func_city_pruning(start_police, graph, run_length, U, pruned_ids_dict):
    """
    Returns a dict of the reachable nodes within the run length for each unit
    """
    unit_nodes = {}
    for u in range(U):
        unit_nodes_ = list(nx.single_source_dijkstra_path(G=graph,
                                       source=start_police[u],
                                       cutoff=run_length,
                                       weight='travel_time'
                                       ).keys()) + [start_police[u]]
        unit_nodes[u] = [pruned_ids_dict[n] for n in unit_nodes_]
...
def sort_nodes(graph, start_fugitive, start_police):
    """
    ...
    for u in range(len(start_police)):
        labels_perunit_sorted[u] = {node: index for index, node in
                                    enumerate(sorted(graph.nodes(), key=lambda n: graph.nodes[n]['distance']))}
        labels_perunit_inv_sorted[u] = {index: node for index, node in enumerate(
            sorted(graph.nodes(), key=lambda n: graph.nodes[n]['distance']))}

    return labels_perunit_sorted, labels_perunit_inv_sorted, labels_sorted
...
def sort_and_filter_pol_fug_city_pruning(graph, start_fugitive, route_fugitive, start_police, run_length):
    """
    Sorts the nodes of a graph on distance to start_fugitive,
    filters the nodes on reachibility by the fugitive,
    and returns the updated labels
