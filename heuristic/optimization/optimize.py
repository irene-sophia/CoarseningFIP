...
# from sort_and_filter import sort_and_filter_pol_fug_city_pruning as sort_and_filter_nodes
    ...
    with MultiprocessingEvaluator(model, n_processes=5) as evaluator:
        for _ in range(1):  # TODO: change to 5
            ...
            result = evaluator.optimize(
                ...
                nfe=2000,  # TODO: change to 20 000
                searchover="levers",
...
if __name__ == '__main__':
    t_max = 1800
    # TODO: deze code is een experiment met optimaliseren op de originele routes
            ...
            for iterations in [1000]:
                for threshold in [0]:
                    ...
                    if graph_type == 'coarsened':
                        G = ox.load_graphml(filepath=f"../networks/panchamy_betweenness_{city}_pruning{pruning}_iter{iterations}_threshold{threshold}.graph.graphml")
                    elif graph_type == 'orig':
                        G = ox.load_graphml(filepath=f"../../data/networks/{city}.graph.graphml")

                    # G_orig = ox.load_graphml(filepath=f"../networks/{city}.graph.graphml")
                    ...
                    # import routes generated on coarsened graph  # TODO
                    with open(f'../simulation/results/betweenness_results_routes_sp_{graph_type}_{city}_pruning{pruning}_iter{iterations}_threshold{threshold}.pkl', 'rb') as f:
                    ...
                    route_data = route_convert(routes_labeled)
                    for pol in police_addresses:
                        node = ox.nearest_nodes(G_orig, pol['lon'], pol['lat'])
                        start_police.append(node)
                    ...
                    print('DONE: ', city, graph_type, pruning, iterations, threshold)
                    # print(len(intercepted_routes))

                    with open(f'../results/betweenness_results_optimization_{city}_{graph_type}_pruning{pruning}_iter{iterations}_threshold{threshold}.pkl', 'wb') as f:
                        pickle.dump(results, f)

                    with open(f'../results/betweenness_results_intercepted_routes_{city}_{graph_type}_pruning{pruning}_iter{iterations}_threshold{threshold}.pkl', 'wb') as f:
                        pickle.dump(intercepted_routes, f)

                    with open(f'../results/betweenness_results_positions_{city}_{graph_type}_pruning{pruning}_iter{iterations}_threshold{threshold}.pkl', 'wb') as f:
